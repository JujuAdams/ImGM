...
GMFUNC(__imgui_dock_space) {
    ...
    GMDEFAULT(0);
    float final_width = static_cast<float>(width);
    float final_height = static_cast<float>(height);
    ImGuiDockNodeFlags flags = (int)YYGetReal(arg, 3);
    ...
    Result.val = ImGui::DockSpace(id, ImVec2(final_width, final_height), flags, final_window_class);
}
...
GMFUNC(__imgui_dock_space_over_viewport) {
    ...
    if (dockspace_id->kind != VALUE_UNDEFINED) {
        final_dockspace_id = YYGetUint32(arg, 0);
    }
    else {
    ...
    unsigned int viewport_id = YYGetUint32(arg, 1);
    GMDEFAULT(0);
    ImGuiViewport* viewport = ImGui::FindViewportByID(viewport_id);
    ImGuiDockNodeFlags flags = (int)YYGetReal(arg, 2);
    GMDEFAULT(ImGuiDockNodeFlags.None);
...
GMFUNC(__imgui_dock_builder_copy_node) {
    ...
    if (node_remap_pairs->kind == VALUE_ARRAY) {
        ...
        for (int i=0; i < len; i++) {
            out_node_remap_pairs->push_back(YYGetUint32(node_remap_pairs, i));
        }
    }
